...
function isChoiceFieldType(type) {
    return (type === "MULTIPLE_CHOICE" || type === "DROPDOWN" || type === "CHECKBOXES");
}
function validateAnswerValue(field, value) {
    ...
    if (isChoiceFieldType(field.type)) {
        if (!Array.isArray(field.options)) {
            throw new Error(`The options for "${field.label}" are not configured correctly`);
        }
        if (field.type === "CHECKBOXES") {
    ...
    if (typeof value === "string") {
        ...
        if (typeof minLength === "number" && value.length < minLength) {
            throw new Error(`"${field.label}" is shorter than the minimum allowed length`);
        }
        ...
        if (typeof pattern === "string") {
            ...
            try {
                ...
                throw new Error(`The validation pattern for "${field.label}" is invalid`);
            }
            if (!regex.test(value)) {
...
function validateFormAnswers(fields, answers) {
    ...
    for (const [key, value] of Object.entries(answers)) {
        ...
        if (field.scope !== "PARTICIPANT") {
            throw new Error(`The field "${field.label}" must be submitted at team level`);
        }
...
function getTeamFields(fields) {
    return fields.filter((field) => field.scope === "TEAM");
}
function validateTeamAnswers(fields, answers, teamName) {
    ...
    for (const field of teamFields) {
        ...
        const value = field.key === "team_name" ? teamName : normalizedAnswers[field.key];
        validateAnswerValue(field, value);
    }
}
async function createRegistrationWithCapacityCheck(eventId, input, userId) {
    return prisma.$transaction(async (tx) => {
        ...
        if (existingRegistration) {
            if (existingRegistration.registrationStatus === "CANCELLED") {
                throw new Error("This email was previously registered and cancelled. Please contact the branch team.");
            }
            throw new Error("This email is already registered for this event");
        }
        const requiresAuthentication = event.access === "MEMBERS_ONLY" || event.access === "UNIVERSITY";
        ...
        if (event.access === "MEMBERS_ONLY") {
            ...
            if (!memberProfile || memberProfile.membershipStatus !== "ACTIVE") {
                throw new Error("An active IEEE GU membership is required for this event");
            }
        }
        ...
        if (!isTeamEvent && input.teamMembers.length > 0) {
            throw new Error("Team members cannot be provided for an individual event");
        }
        if (isTeamEvent) {
            if (!input.teamName) {
        ...
        if (isTeamEvent) {
            ...
            if (uniqueEmails.size !== emails.length) {
                throw new Error("The same email address cannot be used more than once in a team");
            }
            ...
            if (existingRegistration) {
                throw new Error(`The email ${existingRegistration.email} is already registered for this event`);
            }
        }
        let teamId = null;
        ...
        if (isTeamEvent) {
            ...
            if (existingTeam) {
                throw new Error("A team with this name already exists. Please use a different team name.");
            }
            ...
            if (event.maxTeamSize !== null && finalTeamSize > event.maxTeamSize) {
                throw new Error(`Team cannot have more than ${event.maxTeamSize} members`);
            }
            if (event.minTeamSize !== null && finalTeamSize < event.minTeamSize) {
                throw new Error(`Team must have at least ${event.minTeamSize} members`);
            }
        }
        ...
        const availableCapacity = event.capacity === null
            ? null
            : Math.max(event.capacity - registeredCount, 0);
        const registrationStatus = availableCapacity !== null &&
            totalRegistrationsToCreate > availableCapacity
            ? "WAITLISTED"
            : "REGISTERED";
        const createQrToken = () => event.enableQrAttendance ? randomBytes(32).toString("hex") : null;
        ...
        if (isTeamEvent) {
            ...
            const teamNameField = form.fields.find((field) => field.scope === "TEAM" && field.key === "team_name");
            if (teamNameField && !allFormAnswers.team_name) {
                allFormAnswers.team_name = input.teamName;
            }
        }
        const answerEntries = Object.entries(allFormAnswers).filter(([key]) => form.fields.some((field) => field.key === key));
        ...
        for (const member of submittedTeamMembers) {
            const memberRegistration = await tx.eventRegistration.create({
                data: {
                    ...
                    email: member.email,
                    phone: member.phone,
                    qrToken: event.enableQrAttendance && registrationStatus === "REGISTERED"
            ...
            const memberAnswerEntries = Object.entries(member.answers).filter(([key]) => form.fields.some((field) => field.key === key));
        ...
        await tx.auditLog.create({
            data: {
                ...
                description: `Registered for event "${event.title}"${isTeamEvent ? ` with team "${input.teamName}"` : ""}`,
            },
        });
        if (userId) {
...
export async function registerForEvent(eventId, input, userId) {
    const maxRetries = 3;
...
export async function getEventRegistrations(eventId, filters = {}) {
    ...
    const registered = registrations.filter((registration) => registration.registrationStatus === "REGISTERED").length;
    const waitlisted = registrations.filter((registration) => registration.registrationStatus === "WAITLISTED").length;
    const attended = registrations.filter((registration) => registration.registrationStatus === "ATTENDED").length;
    const absent = registrations.filter((registration) => registration.registrationStatus === "ABSENT").length;
    const cancelled = registrations.filter((registration) => registration.registrationStatus === "CANCELLED").length;
...
export async function updateRegistrationStatus(registrationId, status, actorId) {
    const result = await prisma.$transaction(async (tx) => {
        ...
        if (!allowedTransitions.includes(status)) {
            throw new Error(`Invalid registration status transition from ${currentStatus} to ${status}`);
        }
        if (status === "ATTENDED" && registration.event.enableQrAttendance) {
            throw new Error("Attendance must be recorded by scanning the participant QR code");
        }
        ...
        const updatedRegistration = await tx.eventRegistration.update({
            ...
            data: {
                ...
                qrToken: status === "REGISTERED" && registration.event.enableQrAttendance
...
export async function promoteNextWaitlisted(eventId, actorId) {
    const result = await prisma.$transaction(async (tx) => {
        const event = await tx.event.findUnique({
            where: {
                id: eventId,
        ...
        if (event.capacity === null) {
            throw new Error("Waitlist promotion is only applicable to capacity-limited events");
        }
        const activeCount = await tx.eventRegistration.count({
            where: {
                eventId,
...
export async function scanRegistrationQr(qrToken, eventId, actorId) {
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        if (registration.teamId) {
            ...
            const registeredTeamMembers = teamRegistrations.filter((member) => member.registrationStatus === "REGISTERED");
            if (registeredTeamMembers.length === 0) {
                throw new Error("No registered team members are eligible for attendance");
            }
