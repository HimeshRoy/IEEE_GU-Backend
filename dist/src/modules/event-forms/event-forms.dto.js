...
const validationSchema = z
    ...
    if (value.minSelections !== undefined &&
...
export const createEventFormFieldSchema = fieldBaseSchema.superRefine((data, ctx) => {
    const choiceTypes = [
        "MULTIPLE_CHOICE",
        "CHECKBOXES",
        "DROPDOWN",
    ...
    if ((data.type === "FILE_UPLOAD" || data.type === "IMAGE_UPLOAD") &&
    ...
    if (data.type === "IMAGE_UPLOAD" &&
        data.validation?.accept &&
        data.validation.accept.some((value) => !value.toLowerCase().startsWith("image/") &&
...
export const reorderEventFormFieldsSchema = z.object({
    fieldIds: z
        ...
        .min(1)
        .max(200)
        .refine((fieldIds) => new Set(fieldIds).size === fieldIds.length, "Field IDs must be unique"),
});
...
//# sourceMappingURL=event-forms.dto.js.map