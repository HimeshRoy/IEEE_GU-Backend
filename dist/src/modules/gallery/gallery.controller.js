import { addGalleryImageSchema, createAlbumSchema, galleryQuerySchema, updateAlbumSchema, updateGalleryImageSchema, } from "./gallery.dto.js";
import { addGalleryImage, createAlbum, deleteAlbum, deleteGalleryImage, getAlbumById, getAlbums, updateAlbum, updateGalleryImage, uploadAlbumCover, } from "./gallery.service.js";
...
function getErrorStatus(message) {
    if (message === "Gallery album not found" ||
...
export async function getAlbumsController(req, res) {
    try {
...
export async function getManagementAlbumsController(req, res) {
    try {
        const filters = galleryQuerySchema.parse(req.query);
        const albums = await getAlbums({
...
export async function getAlbumController(req, res) {
    try {
...
export async function getManagementAlbumController(req, res) {
    try {
        const albumId = getParam(req.params.albumId);
        if (!albumId) {
            return res.status(400).json({
...
export async function updateAlbumController(req, res) {
    try {
        const albumId = getParam(req.params.albumId);
        if (!albumId) {
            return res.status(400).json({
...
export async function addGalleryImageController(req, res) {
    try {
        const albumId = getParam(req.params.albumId);
        if (!albumId) {
            return res.status(400).json({
        ...
        const image = await addGalleryImage(req.user.id, albumId, input, req.file);
        return res.status(201).json({
            success: true,
...
export async function updateGalleryImageController(req, res) {
    try {
        ...
        const image = await updateGalleryImage(req.user.id, imageId, input);
        return res.status(200).json({
            success: true,
...
export async function deleteGalleryImageController(req, res) {
    try {
        const imageId = getParam(req.params.imageId);
        if (!imageId) {
            return res.status(400).json({
        ...
        const result = await deleteGalleryImage(req.user.id, imageId);
        return res.status(200).json({
            success: true,
...
export async function uploadAlbumCoverController(req, res) {
    try {
        const albumId = getParam(req.params.albumId);
        if (!albumId) {
            return res.status(400).json({
        ...
        const album = await uploadAlbumCover(req.user.id, albumId, req.file);
        return res.status(200).json({
            success: true,
