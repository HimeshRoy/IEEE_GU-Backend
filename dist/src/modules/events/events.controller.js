...
import { createEventSchema, eventListQuerySchema, rejectEventSchema, updateEventSchema, } from "./events.dto.js";
import { approveEvent, cancelEvent, completeEvent, createEvent, deleteEvent, getStudentEvents, getEventById, getEvents, publishEvent, rejectEvent, getEventBySlug, submitEventForApproval, updateEvent, uploadEventBanner, removeEventBanner, } from "./events.service.js";
...
export async function getEventsController(req, res) {
    try {
        ...
        const isStaff = req.user?.role === "WEBMASTER" ||
...
export async function getEventManagementController(req, res) {
    try {
        const query = eventListQuerySchema.parse(req.query);
        const userId = req.user?.id;
        if (!userId) {
...
export async function uploadEventBannerController(req, res) {
    try {
        const userId = req.user?.id;
        const eventId = getEventId(req);
        if (!userId) {
...
export async function removeEventBannerController(req, res) {
    try {
        const userId = req.user?.id;
        const eventId = getEventId(req);
        if (!userId) {
...
export async function submitEventController(req, res) {
    try {
        const userId = req.user?.id;
        const eventId = getEventId(req);
        if (!userId) {
...
export async function approveEventController(req, res) {
    try {
        const userId = req.user?.id;
        const eventId = getEventId(req);
        if (!userId) {
...
export async function rejectEventController(req, res) {
    try {
        const userId = req.user?.id;
        const eventId = getEventId(req);
        if (!userId) {
...
export async function completeEventController(req, res) {
    try {
        const userId = req.user?.id;
        const eventId = getEventId(req);
        if (!userId) {
...
export async function getStudentEventsController(req, res) {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({
