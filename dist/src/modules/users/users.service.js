...
import { deleteImageFromCloudinary, uploadProfileImageToCloudinary, } from "../../services/cloudinary.service.js";
...
async function hasAdministrativeAccess(actorId) {
...
async function canManageUser(actorId, targetUserId) {
...
function validateRoleChange(actorRole, currentRole, newRole) {
...
function validateLeadershipEligibility(targetUser, position) {
    if (!targetUser.isActive) {
        throw new Error("Inactive users cannot be assigned leadership positions");
    }
    const isStudentPosition = STUDENT_LEADERSHIP_POSITIONS.includes(position);
    if (isStudentPosition) {
        if (targetUser.role !== position) {
            throw new Error(`${position} position can only be assigned to a user with the same role`);
        }
        if (!targetUser.memberProfile ||
            targetUser.memberProfile.membershipStatus !== "ACTIVE") {
            throw new Error("Student leadership positions require an active IEEE GU membership");
        }
    }
...
function hasProfileManagementAccess(user) {
    ...
    if (user.role === "FACULTY_MEMBER") {
        return user.leadershipPositions.some((leadership) => leadership.position === "FACULTY_MEMBER");
    }
    return (STUDENT_LEADERSHIP_POSITIONS.includes(user.role) &&
        user.leadershipPositions.some((leadership) => leadership.position === user.role));
}
export async function updateOwnProfile(userId, input) {
    ...
    if (!hasProfileManagementAccess(existingUser)) {
        throw new Error("Only branch leadership members and authorized faculty can update profiles");
    }
    ...
    if (isFacultyProfile &&
        ...
        throw new Error("Department, course, year, and roll number are not available for faculty accounts");
    }
    const updatedUser = await prisma.$transaction(async (tx) => {
        ...
        if (existingUser.memberProfile && !isFacultyProfile) {
            ...
            input.year !== undefined ||
            input.rollNumber !== undefined) {
            throw new Error("Member profile information is not available for this account");
        }
...
export async function changeOwnPassword(userId, input) {
    ...
    if (!hasProfileManagementAccess(user)) {
        throw new Error("Only branch leadership members and authorized faculty can change passwords");
    }
    const validPassword = await argon2.verify(user.password, input.currentPassword);
    if (!validPassword) {
...
export async function createPrivilegedUser(actorId, input) {
    if (!(await hasAdministrativeAccess(actorId))) {
        throw new Error("You are not authorized to create privileged users");
    }
    if (input.role === "WEBMASTER") {
        throw new Error("WEBMASTER accounts cannot be created through this operation");
    }
    const actor = await prisma.user.findUnique({
        where: {
            id: actorId,
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        await tx.auditLog.create({
            data: {
                ...
                entityType: "USER",
                entityId: user.id,
                description: input.role === "STUDENT"
...
export async function updateUserRole(actorId, targetUserId, input) {
    if (!(await hasAdministrativeAccess(actorId))) {
    ...
    const updatedUser = await prisma.$transaction(async (tx) => {
        ...
        if (STUDENT_LEADERSHIP_POSITIONS.includes(input.role)) {
        ...
        for (const leadership of targetUser.leadershipPositions) {
            ...
            if (leadership.position === "IEEE_COUNSELOR" &&
                input.role !== "IEEE_COUNSELOR") {
                positionRemainsValid = false;
            }
            if (leadership.position === "FACULTY_MEMBER" &&
                input.role !== "FACULTY_MEMBER") {
                positionRemainsValid = false;
            }
            if (STUDENT_LEADERSHIP_POSITIONS.includes(leadership.position) &&
...
export async function setUserActiveStatus(actorId, targetUserId, isActive) {
    if (!(await hasAdministrativeAccess(actorId))) {
        throw new Error("You are not authorized to modify users");
    }
    const targetUser = await prisma.user.findUnique({
...
export async function listUsers(actorId) {
    if (!(await hasAdministrativeAccess(actorId))) {
...
export async function assignLeadershipPosition(actorId, input) {
    if (!(await hasAdministrativeAccess(actorId))) {
    ...
    if (existingPosition) {
        throw new Error("This leadership position is already assigned for this academic year");
    }
...
export async function updateLeadershipPosition(actorId, leadershipId, input) {
    if (!(await hasAdministrativeAccess(actorId))) {
...
export async function removeLeadershipPosition(actorId, leadershipId) {
    if (!(await hasAdministrativeAccess(actorId))) {
...
export async function uploadUserProfileImage(userId, file) {
    ...
    const isPrivilegedRole = user.role === "WEBMASTER" ||
        user.role === "IEEE_COUNSELOR" ||
        user.role === "FACULTY_ADVISOR" ||
        user.role === "FACULTY";
    const hasFacultyMemberPosition = user.role === "FACULTY_MEMBER" &&
        user.leadershipPositions.some((leadership) => leadership.position === "FACULTY_MEMBER");
    const hasStudentLeadershipPosition = STUDENT_LEADERSHIP_POSITIONS.includes(user.role) &&
        user.leadershipPositions.some((leadership) => leadership.position === user.role);
    if (!isPrivilegedRole &&
        !hasFacultyMemberPosition &&
        !hasStudentLeadershipPosition) {
        throw new Error("Only branch leadership members and authorized faculty can upload profile photos");
    }
    if (!file) {
    ...
    let uploadResult;
    try {
        uploadResult = await uploadProfileImageToCloudinary(file.buffer, file.originalname);
    }
    catch {
    ...
    try {
        ...
        if (user.profileImage) {
            ...
            if (oldPublicId) {
                try {
                    ...
                    console.error(`Failed to delete previous profile image ${oldPublicId}:`, error);
                }
            }
        }
        return toUserManagementResult(updatedUser);
        ...
        try {
            ...
            console.error(`Failed to clean up profile image ${uploadResult.publicId}:`, cleanupError);
        }
        throw error;
    }
}
...
export async function listMembers(actorId) {
    if (!(await hasAdministrativeAccess(actorId))) {
