...
if (!brevoApiKey) {
  console.warn(
    "BREVO_API_KEY is not configured. Email sending will be disabled.",
  );
}

if (!senderEmail) {
  console.warn(
    "BREVO_SENDER_EMAIL is not configured. Email sending will be disabled.",
  );
}
...
export async function sendEmail({
  ...
  if (!brevo || !senderEmail) {
    console.warn(
      `Email not sent to ${to}: Brevo email configuration is missing.`,
    );
...
function formatEventTime(startTime: Date | null, endTime: Date | null) {
