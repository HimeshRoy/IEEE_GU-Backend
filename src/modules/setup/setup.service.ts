...
export async function createInitialWebmaster(
  ...
  if (!configuredSecret) {
    throw new Error(
      "Initial setup is not configured on this server",
    );
  }
    ...
    async (tx) => {
      ...
      await tx.auditLog.create({
        data: {
          ...
          entityId: user.id,
          description:
            "Initial WEBMASTER account and WEBMASTER leadership position created",
        },
      });
