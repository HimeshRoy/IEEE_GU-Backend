...
export async function createAlbum(userId: string, input: CreateAlbumInput) {
  ...
  await createAuditLog(
    ...
    "GalleryAlbum",
    album.id,
    `Created gallery album "${album.title}"`,
  );

  return album;
}

export async function getAlbums(filters: GalleryListFilters = {}) {
...
export async function updateAlbum(
  ...
  await createAuditLog(
    ...
    "GalleryAlbum",
    album.id,
    `Updated gallery album "${album.title}"`,
  );
...
export async function deleteAlbum(userId: string, albumId: string) {
  ...
  for (const image of existingAlbum.images) {
    try {
      ...
      console.error(
        `Failed to delete Cloudinary image ${image.cloudinaryId}:`,
        error,
      );
    }
  }
  ...
  await createAuditLog(
    ...
    "GalleryAlbum",
    albumId,
    `Deleted gallery album "${existingAlbum.title}"`,
  );
...
export async function addGalleryImage(
  ...
  try {
    ...
    await createAuditLog(
      ...
      "GalleryImage",
      image.id,
      `Added image to gallery album "${album.title}"`,
    );
    ...
    try {
      ...
      console.error(
        `Failed to clean up Cloudinary image ${uploadResult.publicId}:`,
        cleanupError,
      );
    }
...
export async function updateGalleryImage(
  ...
  await createAuditLog(
    ...
    "GalleryImage",
    image.id,
    `Updated image in gallery album "${existingImage.album.title}"`,
  );
...
export async function deleteGalleryImage(userId: string, imageId: string) {
  ...
  await createAuditLog(
    ...
    "GalleryImage",
    imageId,
    `Deleted image from gallery album "${existingImage.album.title}"`,
  );
  ...
  try {
    ...
    if (existingAlbum.coverCloudinaryId) {
      try {
        ...
        console.error(
          `Failed to delete previous album cover ${existingAlbum.coverCloudinaryId}:`,
          error,
        );
      }
    }
    ...
    await createAuditLog(
      ...
      "GalleryAlbum",
      album.id,
      `Updated cover image for gallery album "${album.title}"`,
    );
    ...
    try {
      ...
      console.error(
        `Failed to clean up Cloudinary album cover ${uploadResult.publicId}:`,
        cleanupError,
      );
    }
