...
async function ensureOperationalAccess(userId: string) {
...
async function ensureEditableEvent(eventId: string) {
  ...
  if (
    ...
    throw new Error(
      "The event form cannot be changed in the current event state",
    );
  }
...
async function ensureFormExists(formId: string) {
...
async function ensureEditableForm(formId: string) {
...
function isTeamTemplate(
  template: EventRegistrationTemplate | null | undefined,
) {
  return (
...
function isIndividualTemplate(
  template: EventRegistrationTemplate | null | undefined,
) {
  return (
    template === "UNIVERSITY_INDIVIDUAL" ||
...
function getTemplateFields(
  ...
  const fields: CreateEventFormFieldInput[] = [
    {
      key: "name",
      label: "Full Name",
      type: "SHORT_ANSWER",
...
function validateTemplateConfiguration(
  ...
  if (isIndividualTemplate(template) && participationType !== "INDIVIDUAL") {
    throw new Error(
      "Individual registration templates require individual participation",
    );
  }
...
export async function getEventFormByEventId(userId: string, eventId: string) {
...
export async function updateEventForm(
  ...
  if (input.template !== undefined) {
    validateTemplateConfiguration(
      input.template,
      form.event.participationType,
      form.event.minTeamSize,
      form.event.maxTeamSize,
    );
  }
...
export async function addEventFormField(
  userId: string,
  formId: string,
...
export async function deleteEventFormField(userId: string, fieldId: string) {
  await ensureOperationalAccess(userId);
...
export async function reorderEventFormFields(
  ...
  if (
    input.fieldIds.some((fieldId) => !existingIds.has(fieldId)) ||
...
export async function publishEventForm(userId: string, formId: string) {
  await ensureOperationalAccess(userId);
  ...
  const hasRequiredName = fields.some(
    (field) =>
      field.key === "name" && field.type === "SHORT_ANSWER" && field.required,
  );

  const hasRequiredEmail = fields.some(
    (field) =>
      field.key === "email" && field.type === "EMAIL" && field.required,
  );
  ...
  if (
    form.event.participationType === "TEAM" &&
    !fields.some(
      (field) =>
        field.scope === "TEAM" && field.key === "team_name" && field.required,
    )
  ) {
...
export async function closeEventForm(userId: string, formId: string) {
  await ensureOperationalAccess(userId);
...
export async function reopenEventForm(userId: string, formId: string) {
  await ensureOperationalAccess(userId);
  ...
  if (form.event.status === "COMPLETED" || form.event.status === "CANCELLED") {
    throw new Error(
      "The event form cannot be reopened in the current event state",
    );
  }
...
export async function archiveEventForm(userId: string, formId: string) {
  await ensureOperationalAccess(userId);
...
export async function deleteEventForm(userId: string, formId: string) {
  await ensureOperationalAccess(userId);
